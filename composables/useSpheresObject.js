const spheres = [
	{ position: { x: 1, y: 1.1, z: 0 }, color: 'green', params: { numArmsA: 40, numArmsB: 40, expA: [40, 30, 10] }, direction: 0.5 },
	{ position: { x: -2, y: 1.1, z: 0 }, color: 'yellow', params: { numArmsA: 4, numArmsB: 8, expA: [30, 20, 10] }, direction: 0.5 },
	{ position: { x: 0, y: 1.1, z: 4 }, color: 'green', params: { numArmsA: 5, numArmsB: 10, expA: [20, 40, 10] }, direction: 0.5 },
	{ position: { x: 0, y: 1.1, z: -5 }, color: 'yellow', params: { numArmsA: 6, numArmsB: 12, expA: [40, 20, 10] }, direction: 0.5 },
	{ position: { x: 3, y: 1.1, z: 1 }, color: 'green', params: { numArmsA: 2, numArmsB: 1, expA: [30, 40, 10] }, direction: 0.5 },
	{ position: { x: 0, y: 0.5, z: 0 }, color: 'red', params: { numArmsA: 2, numArmsB: 2, expA: [10, 10, 10] }, cochonette: true, direction: 0.5 },
]

export const useSpheresObject = () => {
...
const isOverlapping = (newPosition, placedSpheres, minDistance) => {
  for (const existingPosition of placedSpheres) {
    const dx = newPosition.x - existingPosition.x
    const dy = newPosition.y - existingPosition.y
    const dz = newPosition.z - existingPosition.z
